...
import com.symphony.oss.canon.parser.log.Slf4jLogFactoryAdaptor;
...
//  }
  ...
  private void execute(List<File> files) throws GenerationException
  {
    ...
    GenerationContext generationContext = new GenerationContext(new Slf4jLogFactoryAdaptor(),
        outputDir_ + "/target/generated-sources",
        outputDir_ + "/target/proforma-sources", outputDir_ + "/target/proforma-copy");
    
    for (File f : files)
      generationContext.addGenerationSource(f);

    generationContext.process(uriMap_ );
