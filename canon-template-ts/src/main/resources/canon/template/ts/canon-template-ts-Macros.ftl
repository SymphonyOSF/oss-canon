...
<#macro setBaseType varPrefix model>
  <#switch model.elementType>
    <#case "Integer">
      <#switch model.format>
        <#case "int32">
          <@"<#assign ${varPrefix}BaseType=\"number\">"?interpret />
          <#return>
          
         <#default>
          <@"<#assign ${varPrefix}BaseType=\"bigint\">"?interpret />
          <#return>
      </#switch>
...
<#------------------------------------------------------------------------------------------------------
 ...
 #
 # This is probably only needed from _ModelFactory.ts.ftl
 #
 # @param model     A model element.
...
<#------------------------------------------------------------------------------------------------------
 # Generate import of IllegalArgumentException if necessary
 #
 # @param model     A model element representing the class to generate for
 #----------------------------------------------------------------------------------------------------->
<#macro checkLimitsClassImportException model><#list model.fields as field><#if isCheckLimits(field)>
import { IllegalArgumentException } from "canon-runtime-ts";
<#return></#if></#list><#if model.superSchema??><@checkLimitsClassThrowsJavaDoc model.superSchema.baseSchema/></#if></#macro>
...
<#macro generateCreateJsonDomNodeFromField indent field var>
  ...
  <#if field.isComponent>
    <#if field.enum??>
${indent}${var}.addIfNotNull("${field.camelName}", get${field.camelCapitalizedName}().toString());
    <#else>
...
${indent}${var}.addIfNotNull("${field.camelName}", get${field.camelCapitalizedName}().getJsonObject());
          <#else>
${indent}${var}.addIfNotNull("${field.camelName}", get${field.camelCapitalizedName}().getValue());
          </#if>
        </#if>
      </#if>
    </#if>
...
${indent}${var}.addCollectionOfDomNode("${field.camelName}", ${javaGetValuePrefix}get${field.camelCapitalizedName}()${javaGetValuePostfix});

      <#else>
${indent}MutableJson${javaCardinality}  value${javaCardinality} = new MutableJson${javaCardinality}();

${indent}for(${fieldElementType} value : get${field.camelCapitalizedName}())
${indent}{
...
${indent}${var}.add("${field.camelName}", value${javaCardinality});
    ...
    <#else>
${indent}${var}.addIfNotNull("${field.camelName}", get${field.camelCapitalizedName}());
    </#if>
  </#if>
</#macro>
